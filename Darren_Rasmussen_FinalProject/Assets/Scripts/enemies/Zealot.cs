...
public class Zealot : MonoBehaviour {
    ...
    List<GameObject> energyBallList = new List<GameObject>();
    ...
    void OnTriggerEnter2D(Collider2D collider)
    {
        if (collider.gameObject.layer == 11)
        {
            ...
            GameObject addedenergyBall = (GameObject)Instantiate(energyBall, transform.position, lookRot);
            addedenergyBall.GetComponent<energyBall>().m_target = collider.gameObject.transform.position;
            if (dir.x < 0)
            {
                addedenergyBall.GetComponent<SpriteRenderer>().flipY = true;
            }
            energyBallList.Add(addedenergyBall);
